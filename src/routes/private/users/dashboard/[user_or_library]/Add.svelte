...
{#if currentView=="file"}
<div transition:scale class="flex flex-col items-center gap-10 absolute top-15 bottom-15 bg-slate-100 w-full p-10 z-2">
...
<div transition:scale class="flex flex-col items-center gap-10 absolute top-15 bottom-15 bg-slate-100 w-full p-10 z-2">
    <div class="bg-indigo-200 rounded-xl overflow-y-auto h-1/2 w-full p-2 pt-4">
        <div class="flex flex-col gap-2">
            {#each Array(30).fill({term:"aaaaa", meaning:"nnnnn"}) as t}
            <div class="w-full rounded-xl flex gap-1 items-center">
            <div class="text-center w-1/2 bg-white rounded-xl p-2">
    ...
    <div bind:this={scroll} class="w-full bg-white rounded-xl p-1 h-2/5 overflow-y-auto">
    ...
    <div class="w-full bg-indigo-200 rounded-xl">
        {#each res_choices[resIndex] as r}
        <button onclick={()=>{resIndex++;messages[resIndex-1].r=r;if(scroll) scroll.scrollTop = scroll.scrollHeight;if(resIndex<3){message_display.push(messages[resIndex]);if(scroll) scroll.scrollTop = scroll.scrollHeight;}}} class="btn btn-primary btn-outline rounded-xl">
            {r}
        </button>
        {/each}
    </div>
    ...
    <button class="p-1 rounded-xl bg-indigo-200 self-end text-indigo-800" onclick={()=>{currentView="";message_display=[messages[0]];resIndex=0;}}>
        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-8">
            <path stroke-linecap="round" stroke-linejoin="round" d="M6 18 18 6M6 6l12 12" />
          </svg>
    </button>

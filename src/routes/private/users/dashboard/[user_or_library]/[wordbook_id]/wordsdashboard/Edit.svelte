        ...
        <div class="w-full flex sm:grid flex-col grid-cols-2 lg:grid-cols-3 gap-5 sm:gap-x-0 items-center place-items-center relative">
            ...
            <div out:slide={{duration:300}} in:fly={{duration:300, y:20}} class="w-4/5 sm:grow flex flex-col justify-center items-start relative">
                ...
                <form out:fade method="POST" action="?/updateWord" use:enhance={() => {
                    ...
                    <span  class={{"pl-2 pr-2 flex bg-white border-1 border-stone-300 shadow-lg rounded-xl max-w-9/10 -translate-x-3 translate-y-1 z-1":true, "hidden":updating}}>
                    ...
                    <div class={{"px-2 flex w-full shadow-lg border-1 border-stone-300 rounded-3xl bg-white relative":true,"hidden":updating}}>

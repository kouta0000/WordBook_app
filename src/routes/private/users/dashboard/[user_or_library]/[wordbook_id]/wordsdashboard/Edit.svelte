    ...
    <div class="w-full pt-18 pb-15 md:pb-20 flex flex-col min-h-screen gap-5 items-center ">
        ...
        <form method="POST" action="?/deleteWords" use:enhance={() => {
            ...
            <div class="w-full px-4 w-4/5 lg:w-1/2 flex gap-3 justify-center items-center absolute fixed bottom-22 lg:right-5 z-20">
                {#if isChecked}
                <button type="submit" class="btn btn-active bg-pink-400 rounded-3xl grow w-min basis-0">
                    <p class="text-white">削除</p>
                </button>
                {/if}
                <button type="button" onclick={() => isChecked=!isChecked} class="btn btn-active bg-pink-300 rounded-3xl grow w-min basis-0 opacity-90">
                    <p class="whitespace-nowrap text-white">{ !isChecked? "選択": "元に戻す"}</p>
                </button>
                {#if !isChecked}
                <button type="button" class="btn btn-active bg-pink-300 rounded-3xl grow w-min basis-0 opacity-90" onclick={() => dialog?.showModal()}>
            ...
            <div out:slide={{duration:300}} in:fly={{duration:300, y:20}} class="w-4/5 sm:grow flex flex-col justify-center items-start">
                ...
                <div class="flex w-full shadow-lg rounded-3xl bg-white border-1 border-sky-300 relative">
                    ...
                    <input type="checkbox" name="deletecheck" value={word.id} class={{"checkbox-xl my-auto mr-2 checkbox checkbox-secondary z-10":true, "hidden":!isChecked, "block":isChecked}}>

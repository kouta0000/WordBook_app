    ...
    <div class="w-full md:w-4/5 lg:w-2/5 mx-auto  lg:h-full flex flex-col bg-white rounded-2xl shadow-xl relative">
        ...
        <div class="w-full mt-1 p-8 flex flex-col justify-evenly gap-5 h-full">
            ...
            <div class="w-full bg-gray-100 rounded-xl flex flex-wrap gap-3 p-5">
                {#each buttons as b,i}
                <button onclick={()=>checkAnswer(b,i)} class={{"text-indigo-800 bg-linear-to-br from-indigo-100 to-gray-100 p-2 text-md font-bold rounded-3xl transition-all duration-200":true,"opacity-0 btn-disabled btn-sm":inputedphrase.includes(b,0)}}>
                    {b}
                </button>
                {/each}
            </div>

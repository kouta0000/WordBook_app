    ...
    <div class="w-full pt-16 pb-15 md:pb-20 flex flex-col min-h-screen gap-2 items-center">
        ...
        <div class="w-full flex sm:grid flex-col grid-cols-2 lg:grid-cols-3 gap-5 sm:gap-x-0 items-center place-items-center relative">
            ...
            <div bind:this={cards[i]} out:slide={{duration:300}} in:fly={{duration:300, y:20}} class="w-9/10 sm:grow flex flex-col justify-center items-start relative">
                <div class="flex justify-center w-full shadow-lg bg-white shadow-sm rounded-t-xl rounded-r-xl relative">
                    ...
                    -->
                        <div style={parent_style} class="w-full">
                            <p use:fit={{min_size:5, max_size:22}} class="font-sans font-semibold pl-5 pr-1 pt-5 pb-1 text-2xl">{!soundmode? (isFlipped? word.meaning: word.term) : `${word.term[0]}${Array(word.term.length-1).fill("_").join("")}`}</p>
                        </div>
                        <div class="flex w-full p-1 h-9">
                        {#if shows[i]}
                        <div style={parent_style}>
                        <p use:fit={{min_size:17, max_size:25}} transition:fade={{duration:300}} class="text-gray-500 text-right font-sans p-1 pr-2">
                    ...
                    <div class="w-full relative transition-all duration-200 rounded-b-3xl">
                        <div transition:slide class=" w-9/10 rounded-b-3xl bg-linear-to-r from-gray-200 to-indigo-200 overflow-hidden flex self-start">
                            <button disabled onclick={() => {showPhrases[i]=true;showPhrases.forEach((v,id)=>{if(id!=i)showPhrases[id]=false});currentviews[i]="synonym"; displays[i] = fetchtext(word.term,"synonym",word.id,false)}} class="btn btn-disabled btn-sm bg-white/10 text-indigo-100 font-black w-1/3">
                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-6">
                                    <path stroke-linecap="round" stroke-linejoin="round" d="M15 19.128a9.38 9.38 0 0 0 2.625.372 9.337 9.337 0 0 0 4.121-.952 4.125 4.125 0 0 0-7.533-2.493M15 19.128v-.003c0-1.113-.285-2.16-.786-3.07M15 19.128v.106A12.318 12.318 0 0 1 8.624 21c-2.331 0-4.512-.645-6.374-1.766l-.001-.109a6.375 6.375 0 0 1 11.964-3.07M12 6.375a3.375 3.375 0 1 1-6.75 0 3.375 3.375 0 0 1 6.75 0Zm8.25 2.25a2.625 2.625 0 1 1-5.25 0 2.625 2.625 0 0 1 5.25 0Z" />
                                  </svg>                                  
                            </button>
                            <button disabled onclick={() => {showPhrases[i]=true;showPhrases.forEach((v,id)=>{if(id!=i)showPhrases[id]=false});currentviews[i]="collocation";displays[i] = fetchtext(word.term,"collocation",word.id,false)}} class="btn btn-disabled btn-sm bg-white/10 text-indigo-100 font-black w-1/3">
                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-6">
                ...
                {#if showPhrases[i]}
                <div transition:slide class={{"absolute top-[100%] left-[50%] -translate-x-[50%] z-3 w-[98vw] md:w-[33vw] overflow-hidden bg-linear-to-br from-indigo-200/90 to-indigo-200/80 rounded-3xl flex flex-col transition-all duration-200":true}}>
